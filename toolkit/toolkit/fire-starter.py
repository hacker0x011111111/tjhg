...
def assetfinder(args, home_dir, thisFqdn, logger):
    try:
        subprocess.run(["assetfinder --subs-only {args.fqdn} > ./temp/assetfinder.tmp"], shell=True)
...
def gau(args, home_dir, thisFqdn, logger):
    try:
        subprocess.run(["gau --subs {args.fqdn} | cut -d / -f 3 | sort -u > ./temp/gau.tmp"], shell=True)
...
def masscan_to_tls(home_dir):
    print(1)
    #rocess.run([f"cat ./temp/tls-scan-in.tmp | {home_dir}/Tools/tls-scan/tls-scan --port=443 --concurrency=150 --cacert={home_dir}/Tools/tls-scan/ca-bundle.crt 2>/dev/null -o wordlists/tls-results.json"], shell=True)
...
def collect_screenshots(home_dir, thisFqdn, logger):
    ...
    subprocess.run([f"norter-t {home_dir}/nuclei-templates/headless/screenshot.yaml -l ./temp/urls.txt -stats -system-resolvers -config config/nuclei_config.yaml -vv --headless -sb -hbs 10 -headc 1 -fhr -hm"], shell=True)
