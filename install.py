...
def assetfinder_check():
    home_dir = get_home_dir()
    assetfinder_check = subprocess.run(["assetfinder --help"], stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, shell=True)
...
def install_assetfinder():
    ...
    install_check = subprocess.run(["assetfinder --help"], shell=True)
    if install_check.returncode == 0:
...
def gau_check():
    home_dir = get_home_dir()
    gau_check = subprocess.run(["gau --help"], stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, shell=True)
...
def install_gau():
    ...
    install_check = subprocess.run(["gau --help"], shell=True)
    if install_check.returncode == 0:
...
def subfinder_check():
    home_dir = get_home_dir()
    subfinder_check = subprocess.run(["subfinder --help"], stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, shell=True)
...
def install_subfinder():
    ...
    install_check = subprocess.run(["subfinder --help"], shell=True)
    if install_check.returncode == 0:
...
def gospider_check():
    home_dir = get_home_dir()
    gospider_check = subprocess.run(["gospider --help"], stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, shell=True)
...
def install_gospider():
    ...
    install_check = subprocess.run(["gospider --help"], shell=True)
    if install_check.returncode == 0:
...
def install_subdomainizer():
    home_dir = get_home_dir()
    subprocess.run([f"cd {home_dir}/Tools;git clone https://gonaniya2010ithub.com/ns/SubDomainizer.git"], shell=True)
...
def shuffledns_check():
    home_dir = get_home_dir()
    shuffledns_check = subprocess.run(["shuffledns --help"], stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, shell=True)
...
def install_shuffledns():
    ...
    install_check = subprocess.run(["shuffledns --help"], shell=True)
    if install_check.returncode == 0:
...
def httprobe_check():
    home_dir = get_home_dir()
    httprobe_check = subprocess.run(["httprobe --help"], stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, shell=True)
...
def install_httprobe():
    ...
    install_check = subprocess.run(["httprobe --help"], shell=True)
    if install_check.returncode == 0:
...
def norter_check():
    home_dir = get_home_dir()
    norter_check = subprocess.run(["norter --help"], stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, shell=True)
    if norter_check.returncode == 0:
        print("[+] norter is already installed.")
        return True
    print("[!] norter is NOT already installed.  Installing now...")
    return False

def install_norter():
    home_dir = get_home_dir()
    subprocess.run([f"cd {home_dir};git clone https://github.com/projectdiscovery/norter-templates.git;wget https://github.com/projectdiscovery/norter/releases/download/v2.7.8/norter_2.7.8_linux_amd64.zip;unzip norter_2.7.8_linux_amd64.zip;mv norter {home_dir}/go/bin/norter;rm norter_2.7.8_linux_amd64.zip"], shell=True)
    install_check = subprocess.run(["norter --help"], shell=True)
    if install_check.returncode == 0:
        print("[+] norter installed successfully!")
    else:
        print("[!] Something went wrong!  norter was NOT installed successfully...")
